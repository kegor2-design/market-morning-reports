...
[[ "${MMP_YOUTUBE_INSIGHT_ENABLED:-0}" == "1" ]] || { echo "[STOP] MMP_YOUTUBE_INSIGHT_ENABLED must be 1" >&2; exit 2; }
...
# Foreground by default. The pipeline now writes both view cards and cross-channel nightly synthesis.
exec "${MMP_PYTHON:-python3}" -m market_morning_publisher.youtube_insight.cli "${ARGS[@]}" "$@" \
  2>&1 | tee -a "$PROJECT_DIR/logs/nightly_youtube_intelligence.log"
