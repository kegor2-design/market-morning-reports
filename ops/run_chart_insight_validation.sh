...
# Existing reviewed/local OHLCV is used by default. Pass --fetch-yahoo explicitly for missing historical bars.
exec "${MMP_PYTHON:-python3}" -m market_morning_publisher.chart_insight.cli --root "$PROJECT_DIR" "$@" \
  2>&1 | tee -a "$PROJECT_DIR/logs/chart_insight_validation.log"
